...
public class StripesDomainTest {
    ...
    private static class TestDomain
        ...
        @Override
        public TestDomain assign(final Identifier id, final ValueExpression expression, final ProgramPoint pp) throws SemanticException {
            ...
            if ((pp instanceof Assignment) && (id instanceof Variable variable)) {
                ...
                if (result1 == null) {
                    ...
                    Assert.assertThat("Constants are different in expression " +expression, result2.getConstant(),
                            is(constant));
                    
                    if ((firstVariable == null) && (secondVariable == null)) {
                        Assert.assertThat("Result is not constant in expression " +expression, result2.isConstant(), is(true));
                    } else if ((firstVariable != null) && (secondVariable == null)) {
                        Assert.assertThat("Monomials are different in expression " + expression, result2.getSize(),
                                is((firstIdentifierCount == 0) ? 0 : 1));
                        if (firstIdentifierCount != 0) {
                            final Monomial monomial = result2.getMonomial(0);
                            Assert.assertThat("Monomials are different in expression " + expression, monomial.getVariable(),
                                    is(firstVariable));
                            Assert.assertThat("Monomials are different in expression " + expression, monomial.getCoefficient(),
                        ...
                        if (firstMonomial.getVariable().equals(firstVariable)) {
                            Assert.assertThat("First Monomials are different in expression " +expression, firstMonomial.getVariable(),
                                    is(firstVariable));
                            Assert.assertThat("First Monomials are different in expression " +expression, firstMonomial.getCoefficient(),
                                    is(firstIdentifierCount));
                            Assert.assertThat("Second Monomials are different in expression " +expression, secondMonomial.getVariable(),
                                    is(secondVariable));
                            Assert.assertThat("Second Monomials are different in expression " +expression, secondMonomial.getCoefficient(),
                                    is(secondIdentifierCount));
                        } else {
                            Assert.assertThat("First Monomials are different in expression " +expression, secondMonomial.getVariable(),
                                    is(firstVariable));
                            Assert.assertThat("First Monomials are different in expression " +expression, secondMonomial.getCoefficient(),
                                    is(firstIdentifierCount));
                            Assert.assertThat("Second Monomials are different in expression " +expression, firstMonomial.getVariable(),
                                    is(secondVariable));
                            Assert.assertThat("Second Monomials are different in expression " +expression, firstMonomial.getCoefficient(),
                                    is(secondIdentifierCount));
                        }
                    }
                }
        ...
        @Override
        public TestDomain smallStepSemantics(final ValueExpression expression, final ProgramPoint pp) throws SemanticException {
        
            return this;
        }
    
        @Override
        public TestDomain assume(final ValueExpression expression, final ProgramPoint pp) throws SemanticException {
        ...
        @Override
        public Satisfiability satisfies(final ValueExpression expression, final ProgramPoint pp) throws SemanticException {
